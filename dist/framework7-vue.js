    ...
    render: function (c) {
      ...
      if (self.type === 'select' || self.type === 'textarea' || self.type === 'file') {
        ...
        if (self.type === 'select') {
          ...
          inputEl = c('input', {attrs: attrs, on: on, style: self.inputStyle}, self.$slots.default);
        }
        else {
          inputEl = c('textarea', {attrs: attrs, on: on, style: self.inputStyle, class: {resizable: self.resizable}, domProps: {value: self.valueComputed}}, self.$slots.default);
        }
      }
      else {
        if(self.$slots.default && self.$slots.default.length > 0 || !self.type) {
          ...
          else { inputEl = c('input', {attrs: attrs, on: on, style: self.inputStyle, domProps: {value: self.valueComputed, checked: self.checkedComputed}}); }
        }
      }
